"""Logging configuration for moltrade."""
...
__all__ = ["get_logger", "use_moltrade_log_handler", "set_log_level"]
...
_PACKAGE_COLOR = {
    "moltrade": _Ansi.CYAN,
    "nostr": _Ansi.GREEN,
    "exchanges": _Ansi.YELLOW,
    "strategies": _Ansi.MAGENTA,
}
...
def _initialize_if_necessary():
    ...
    with _init_lock:
        ...
        _enable_default_handler(("__main__", "moltrade", "nostr", "exchanges", "strategies"))
...
def get_logger(name: Optional[str] = None) -> logging.Logger:
    """
    Grab a logger with moltrade’s default handler attached.
...
def use_moltrade_log_handler(for_root: bool = True):
    """
    ...
    Example
    -------
        from logging_config import use_moltrade_log_handler
        use_moltrade_log_handler()  # now requests, hivemind, etc. share the style
    """
    del for_root
    _initialize_if_necessary()
    root = logging.getLogger()
